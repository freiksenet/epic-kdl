...
use crate::ast::{Literal, Value, Integer, Decimal, Radix, TypeName, BuiltinType};
...
macro_rules! impl_integer {
    ($typ: ident, $marker: ident) => {
        ...
        impl<S: ErrorSpan> DecodeScalar<S> for $typ {
            fn raw_decode(val: &Value<S>, ctx: &mut Context<S>)
                ...
                match &*val.literal {
                    Literal::Int(ref value) => {
                        match value.try_into() {
                            Ok(val) => Ok(val),
                            Err(e) => {
                                ctx.emit_error(DecodeError::conversion(&val.literal, e));
                                Ok(0)
                            }
                        }
                    }
...
impl<S: ErrorSpan> DecodeScalar<S> for String {
        ...
        -> Result<String, DecodeError<S>>
    {
        match &*val.literal {
            ...
            _ => {
                ctx.emit_error(DecodeError::scalar_kind(Kind::String, &val.literal));
                Ok(String::new())
            }
        }
    }
...
impl<S: ErrorSpan> DecodeScalar<S> for bool {
        ...
        -> Result<bool, DecodeError<S>>
    {
        match &*val.literal {
            ...
            _ => {
                ctx.emit_error(DecodeError::scalar_kind(Kind::Bool, &val.literal));
                Ok(Default::default())
            }
        }
    }
